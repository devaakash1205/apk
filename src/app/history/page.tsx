  ...
  return (
    <Card>
      <CardContent className="p-4 flex items-center justify-between">
        ...
        <p className={`font-semibold ${isSuccess ? "text-foreground" : "text-muted-foreground line-through"}`}>
    ...
    <div className="p-4 space-y-4">
      ...
      {Object.keys(groupedTransactions).length > 0 ? (
        Object.entries(groupedTransactions).map(([date, txs]) => (
          <div key={date}>
            <h2 className="text-sm font-medium text-muted-foreground mb-2 px-2">{date}</h2>
